...
LLM 不直接决定出版状态、CCF-A/控制工程资格、跨源去重、核心排序和最终选文。系统有两类独立的 LLM 数据流：首次编译画像或画像指纹变化时，画像编译器会把完整自然语言 `profile.description` 发送给配置的 LLM，以提取受严格 Schema 约束的研究语义；论文卡片生成器则只处理最终入选论文的标题、摘要和已编译兴趣。数据源、Venue、Category 与配额最终由确定性映射、用户显式覆盖和本地校验决定，而不是由 LLM 任意执行。
...
- OpenAlex 接收候选 DOI；
- 画像编译 LLM 在首次编译或画像/覆盖/数量指纹变化时接收完整自然语言画像；指纹未变化时直接使用私人缓存，不重复发送；
- 论文卡片 LLM 接收最终入选论文的标题、摘要和已编译兴趣方向；这与画像编译是两类独立数据流；
